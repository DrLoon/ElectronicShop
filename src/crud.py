...
def add_product_to_cart(db: Session, product_id: int):
    res = db.query(models.Product_DB).get(product_id)
    if not res:
...
def remove_product_from_cart(db: Session, product_id: int):
    db.query(models.Cart_DB).filter(models.Cart_DB.product_id == product_id).delete()
    db.commit()
    return None


def set_cart_product_amount(db: Session, product_id: int, new_amount: int):
    cart_item = db.query(models.Cart_DB).filter(models.Cart_DB.product_id == product_id).first()
    if not cart_item:
        raise HTTPException(status_code=404, detail=f"There is no such product in the cart")

    pr = db.query(models.Product_DB).get(product_id)
